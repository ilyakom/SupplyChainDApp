...
contract supplyChain {
    ...
    mapping(uint32 => uint32[]) public productTrack; // ownerships by Product ID (product_id) / Movement track for aproduct
    ...
    function newOwner(uint32 _user1Id, uint32 _user2Id, uint32 _prodId) onlyOwner(_prodId) public returns (bool) {
        ...
        || keccak256(abi.encodePacked(p1.participantType)) == keccak256("Supplier") 
            && (keccak256(abi.encodePacked(p2.participantType)) == keccak256("Supplier") || keccak256(abi.encodePacked(p2.participantType))==keccak256("Consumer"))) {
